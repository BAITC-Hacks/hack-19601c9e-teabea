...
def main():
    ...
    with st.sidebar:
        ...
        st.caption("Цвет — роль · размер — priority_score (10–34 px). Золотая обводка — seed, треугольник — boundary.")
    ...
    with graph_tab:
        ...
        with left:
            if visible.empty:
                ...
                components.html(render_graph_html(visible, links, state.selected_gid), height=635, scrolling=True)
        with right:
            ...
            if selected.empty:
                ...
                if r.is_seed:
                    st.warning("Seed: наблюдаемый вход неполон.")
                if r.is_frontier_cutoff:
                    st.warning("Boundary: depth=4. Отсутствие исходящих не доказывает удержание денег.")
    ...
    with limits_tab:
        ...
        st.info("Обход ограничен depth=4; входящие seed неполны. Переводы ниже 5000 KZT и межбанковские переводы отсутствуют. Наблюдаемые суммы не являются полным балансом. Нет ground truth: роли — эвристические гипотезы, role_score не вероятность виновности.")
