...
Локальный анализ наблюдаемой сети переводов. Неизменённый pipeline.py рассчитывает роли, кластеры и приоритеты; Streamlit + PyVis читает готовые CSV. Старые frontend/backend не участвуют в основном запуске и не включены в архив.
...
Цвет — роль, размер — priority_score (10–34 px), золотая обводка — seed, треугольник — boundary. Физика ограничена 120 итерациями и таймером 4 секунды, есть кнопка остановки. Полные gid в pandas/JSON/PyVis — строки. Клик по графу открывает подробности внутри iframe; карточка Streamlit выбирается поиском и топом. Кластеры доступны все.
...
**14 тестов подтверждены**: 6 pipeline, 5 UI, 3 launcher. Успешный запуск run.py и HTTP health 200. Визуальные проверки браузера, 1280×720 и скачивание через браузер не подтверждены: браузерный инструмент недоступен. Подробности: [verification.md](docs/verification.md), [измерения и SHA-256](docs/verification.json), [версии пакетов](docs/tested-environment.txt).
...
Архив: release/graf-deneg-submission.zip. Содержит исправленные результаты и исходные данные, исключает окружения, .git, кэши и старые out.
